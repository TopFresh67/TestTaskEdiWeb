...
When(/^удаляю пользователя с логином (\w+\.\w+)$/) do |login1|

  response = $rest_wrap.delete('/users', login: login1)
  $logger.info(response.inspect)
end

When(/^обновляю данные пользователя с логином (\w+\.\w+) параметрами: имя (\w+) фамилия (\w+) пароль ([\d\w@!#]+)$/) do
|login2, name, surname, password|

  response = $rest_wrap.put('/users', login: login2,
                            name: name,
                            surname: surname,
                            password: password,
                            active: 1)
